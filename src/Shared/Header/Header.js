            ...
            <Container>
                {
                    (currentUser === 'seller' || currentUser === 'admin')  ?
                                ...
                                <Nav className="ms-auto">
                                    {
                                        user?.uid ? <div className=' privateNav'>
                                            ...
                                            <NavLink onClick={(currentUser==='admin')? handleAdminLogOut : handleLogOut} className='navLink pe-4 py-3 fw-bold' style={({ isActive }) => isActive ? { color: 'rgb(107, 67, 251)', textDecorationColor: 'rgb(107, 67, 251)', textDecorationThickness: '3px', textUnderlineOffset: '8px' } : { color: 'black', textDecoration: 'none' }}>LogOut</NavLink></div>
                        ...
                        :
                        <>
                            <Navbar.Brand className='fw-bold '><Link className=' text-decoration-none fw-bold ps-2 ' style={{ fontSize: '2rem', color: '#6B43FB' }} to='/'><span style={{ color: 'black' }}>Car</span>Swap</Link></Navbar.Brand>
                            <Navbar.Toggle className='navToggle' aria-controls="responsive-navbar-nav" />
                            <Navbar.Collapse id="responsive-navbar-nav">
                                <Nav className="ms-auto">
                                    ...
                                    {
                                        user?.uid ? <div className=' privateNav'>
                                            <NavLink className='navLink pe-4 py-3 fw-bold' to='/allcars' style={({ isActive }) => isActive ? { color: 'rgb(107, 67, 251)', textDecorationColor: 'rgb(107, 67, 251) ', textDecorationThickness: '3px', textUnderlineOffset: '8px' } : { color: 'black', textDecoration: 'none' }}>All Cars</NavLink>
                                            <NavLink className='navLink pe-4 py-3 fw-bold' to={`myOrders/${user?.email}`} style={({ isActive }) => isActive ? { color: 'rgb(107, 67, 251)', textDecorationColor: 'rgb(107, 67, 251)', textDecorationThickness: '3px', textUnderlineOffset: '8px' } : { color: 'black', textDecoration: 'none' }}>Dashboard</NavLink>
                                            <div className='d-inline imgDiv'><img className='userProfileImg' onMouseEnter={() => setUserName(true)} onMouseLeave={() => setUserName(false)} style={{ height: '40px', width: '40px', marginRight: '22px', padding: '2px', borderRadius: '50%', objectFit: 'cover', border: '3px solid #6B43FB' }} src={user.photoURL} alt="" /></div>
                                            <p className={userName ? "visible showName rounded" : "visible hideName rounded"}>{user?.displayName}</p>
